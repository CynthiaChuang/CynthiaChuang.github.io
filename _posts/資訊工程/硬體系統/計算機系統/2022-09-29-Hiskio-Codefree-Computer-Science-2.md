...
- **後端**  
    <mark>後端工程師則是負責資料傳遞與網站的溝通層面</mark>，也就是負責**打地基**的，它會將前端的動作傳遞到後端並從資料庫撈取相對應的資料，最後呈現在頁面上。除了熟悉後端程式語言與演算法，也要了解前端相關知識、網路通訊協定、大資料處理，甚至是伺服器的設定...等。
...
函式庫除了依照協助運行程式來區分函式庫外，還可以依<mark>輔助程式語言相關使用</mark>來區分：
