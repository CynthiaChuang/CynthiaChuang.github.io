...
4. **不能運作的狀況**  
     <span class='label'>不要直接更動 array 的內容</span>，即便透過 console.log 和 Vue 工具看資料都是修改過的，但畫面依然顯示舊資料，因為視圖無更新，無效的改動如下：
    ```javascript
    ...
    <br>因為 <span class='label'>Vue 無法探測普通的新增屬性</span>，它必須用於向響應式對像上添加新屬性，因此必須使用 <span class='label'>Vue.set</span> 強制將資料寫入視圖中：
	...
	delete 也是相同情況 <span class='label'>Vue.delete( target, key )</span>。
...
3. **v-if v.s v-show**
    ...
    且 v-if 是<span class='label'>惰性的</span>：如果在初始渲染時條件不成立，就什麼也不做，直到在條件第一次變為成立後才開始局部編譯（編譯會被暫存起來）。

    相比之下，v-show 元素則是全部會被編譯並保留，只是簡單地基於 CSS 切換，設定 style 為 display:none。
    
    ![Imgur](https://i.imgur.com/gJtpP21.png)
    <br>
    一般來說，v-if 有更高的切換消耗而 v-show 有更高的初始渲染消耗。因此，<span class='label'>如果需要頻繁切換，則使用 v-show 較好；如果在運行時條件不大可能改變，則使用 v-if 較好</span>。
...
2. **Computed：**  
	是針對我們要輸入到畫面的內容，在輸出前需要做前處理。簡單來說，當資料出現變化需要<span class='label'>改變畫面</span>的 function 放這裡 。
...
### 事件修飾符
1. **@click.stop**  
    可避免<span class='label'>冒泡事件</span>。所謂的冒泡事件是指，點擊事件觸發後在 DOM 裡面傳遞的時會，從子元素不斷往其父元素傳遞，就像泡泡浮上水面一樣。 
